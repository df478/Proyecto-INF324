...
$verificar_fecha_fin = "SELECT fecha_fin FROM `seguimiento` 
                        WHERE proceso LIKE (select proceso from flujoauditoria where siguiente like 'Evaluación del Control Interno') 
...
if ($resultado && $resultado['fecha_fin'] !== null) {
    ...
    if (isset($_POST['accion']) && $_POST['accion'] === 'anadir' && isset($_POST['descripcion_objetivo'])) {
        ...
        SELECT (flujo + 1), 'Evaluación del Control Interno', :usuario, :fecha_inicio, NULL
        FROM `seguimiento` 
...
$query = "SELECT * FROM datosauditoria.controlinterno where EvaluacionControlInterno is not null";
